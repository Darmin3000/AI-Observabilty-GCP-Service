...
# Dynamic Alert Policy Configuration Service

## Overview

This document describes the **Alert Policy Configuration Service**, an extension to the alerting system that programmatically manages **Google Cloud Monitoring alert policies**.

The service dynamically generates and maintains alert rules using Python. It supports two sources of alert thresholds:

1. **Default thresholds defined in code**
2. **Dynamic threshold overrides stored in Cloud SQL**

This design enables centralized configuration while maintaining safe fallback behavior when database overrides are not available.

---

# Motivation

In large monitoring systems (such as ML monitoring platforms), alert thresholds may vary across models or metrics. Hardcoding alert rules in the monitoring platform can create operational challenges:

- Difficult to maintain large numbers of alerts
- Inconsistent thresholds across environments
- Limited flexibility for per-model customization
- Manual configuration drift

The Alert Policy Configuration Service solves these problems by generating alert policies programmatically.
...
# Threshold Resolution Strategy

The system supports **multi-level threshold resolution**.

When generating alert policies, thresholds are determined in the following priority order:

1. **Model ID + Metric override**
2. **Model Name + Metric override**
3. **Default metric threshold (fallback)**

Example resolution order:

```
Model-specific threshold (highest priority)
        ↓
Model-name threshold
        ↓
Default metric threshold (fallback)
```

This ensures alerts are always generated even when the database is unavailable.

---

# Default Thresholds

Default thresholds are defined directly in the service code.
...
These values serve as the **fallback configuration** if no database override exists.

---

# Cloud SQL Threshold Overrides

The service optionally retrieves threshold overrides from **Cloud SQL**.

This enables per-model configuration without redeploying code.
...
| metric_name | TEXT | Monitoring metric |
| threshold_value | FLOAT | Threshold value |
| severity | TEXT | Alert severity level |

Example rows:

| model_id | model_name | metric_name | threshold_value |
|---------|------------|------------|----------------|
| 101 | churn_model | drift_score | 0.75 |
| 102 | fraud_model | accuracy_drop | 0.65 |

---

# Threshold Resolution Logic

When generating alert rules, the system resolves thresholds as follows:

1. Check if a **model_id + metric** override exists
2. If not, check for a **model_name + metric** override
3. If neither exists, use the **default threshold**

Pseudo logic:

```
if (model_id, metric) exists in database:
    use database threshold
else if (model_name, metric) exists in database:
    use database threshold
else:
...
Alert policies are created programmatically using the **Google Cloud Monitoring API**.

Each alert rule monitors a specific metric for a specific model.

Example policy structure:

```
Metric: custom.googleapis.com/model/drift_score
...
Generated alert policies integrate directly with the existing alert routing system.
...
This ensures that dynamically generated alerts continue to use the same notification pipeline.

---

# Notification Channels

Each generated alert policy is configured with a **Pub/Sub notification channel**.

This Pub/Sub topic triggers the existing alert handling service responsible for:
...
The Alert Policy Configuration Service can run in several ways:

### Option 1 — Scheduled Job

Run periodically to synchronize alert policies.
...
Execute during infrastructure deployment.

Example:

```
CI/CD Pipeline → Python Alert Policy Generator
```
...
Trigger updates when thresholds change in the database.

Example:

```
Cloud SQL update → Pub/Sub → Configuration Service
```
...
The service should avoid creating duplicate policies.

Best practice:

```
1. List existing alert policies
2. Check if policy already exists
3. Update existing policy if needed
4. Create policy if it does not exist
```

This ensures consistent alert management without duplication.
...
- **Dynamic per-model alert configuration**
...
- Automatic alert deletion when models are retired
- Versioning of threshold changes
...
The Alert Policy Configuration Service introduces dynamic alert management by combining:

- **Default thresholds defined in code**
- **Cloud SQL threshold overrides**
- **Programmatic alert policy creation using the Cloud Monitoring API**

This approach provides a scalable and maintainable solution for managing alert policies across large numbers of models and metrics.
...
# Google Cloud DLP Layer (PII Detection)

## Overview

The **Data Loss Prevention (DLP) layer** in this system is responsible for detecting **Personally Identifiable Information (PII)** that may appear in the output of a Large Language Model (LLM).

LLMs can sometimes unintentionally generate sensitive information such as:
...
The DLP layer inspects the model’s output and determines whether sensitive information is present. If PII is detected, the pipeline assigns a **PII leakage score** indicating that the model output may violate privacy or compliance policies.

This mechanism helps organizations monitor and prevent **data leakage risks** when deploying generative AI systems.

---

# Role of the DLP Layer in the Evaluation Pipeline

The DLP layer sits inside the **Generative AI evaluation module** and is executed whenever a generative trace is evaluated.

If sensitive data is found, the response is flagged for monitoring and auditing.

---

# What Google Cloud DLP Does

Google Cloud DLP analyzes text to detect **sensitive information types**, called **InfoTypes**.

These InfoTypes are predefined detectors for common categories of personal data.
...
Detection is performed using a combination of:

### Pattern Matching

DLP contains built-in patterns similar to regex for structured data.

---

### Contextual Machine Learning

For less structured data like names or locations, DLP uses machine learning to analyze surrounding text.

Based on the model owner's specifications however, certain PII are expected to be present. These are given a pass, however, the service raises a flag for unexpected PII types.